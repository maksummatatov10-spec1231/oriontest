...
| (чат 3, 2026-09-12) **patch11** | orion_menu_patch11.swf | **эксперимент №1: `patch_core_timestep` выключен, header FPS 60, без принудительного `stage.frameRate=120`** — цикл таймингов байт-в-байт как оригинал/patch7. Меню = то же, что в p10 (5 вкладок, hidden до Insert/F7) | **пользователь: «нажал старт — процесс закрылся полностью»** (первая сборка, где код меню реально отработал — 10.2) |
| (чат 3, 2026-09-12) **patch12** | orion_menu_patch12.swf | **фикс п11: в `add_bmp` width/height ставятся ТОЛЬКО после присвоения bitmapData** (раньше `new Bitmap(); width=36; height=36` при `bitmapData==null` → `scaleX = 36/0 = Infinity` → AIR роняет процесс; это мертвый конец №20, факт-проверка 2026-09-12). Плюс: всё как p11 (без timestep, 60 FPS, меню p10). Пустые слоты сетки 0×0 до do_fill; иконки боссов/мобов размеров не получают, если ключ не найден | **ждём пользователя: старт проходит? мир виден?** |
...
вперёд: 68, 30, 43, 59, 68). **Итог у пользователя: «нажал старт — процесс закрылся
полностью».** Это ПЕРВАЯ сборка, где код меню реально отработал (в p9/p10 тиков не
было вообще — см. 10.2).

ВАЖНО (проверено 2026-09-12): сборка p10 из текущего `tools/build_swfs.py`
(откат 4 правок p11) даёт **байт-в-байт тот же файл**, что закатан в ветку
(git blob `93eac263330661964df7d7506d8cab53b3510e9a`) — конвейер сборки верен,
все сравнения orig/p10/p11/p12 валидны.
...
24. Считать, что `curl raw.githubusercontent.com` работает в песочнице (SSL 35).
    Только `gh api` (2.9).  
25. **width/height на Bitmap до присвоения bitmapData** (подтверждено 2026-09-12:
    p11 закрыл процесс при «старт»; p7 без Bitmap работает; p10 этот код не
    исполнял вообще). Сетевые слоты держать 0×0 до do_fill; иконки — размер только
    если ключ найден. TextField не страдает (пустой TextField = 100×100 по
    умолчанию).
...
## 10.2 ПОЧЕМУ П11 ПАЛО ПРИ «СТАРТ», А П10 — НЕТ (установлено 2026-09-12)

Ключевой довод: **в p9/p10 код меню НИ РАЗУ не исполнялся.** Timestep-патч в
регионе 33..53 (`_prevFixedUpdateTime = now`) выполняется **каждый кадр
безусловно** — включая кадры ЛОББИ. К моменту входа в мир `_prevFixedUpdateTime`
свежий (now предпоследнего кадра лобби) → первый кадр мира: `local3 = 8.3ms <
FR` → **0 тиков вообще, не только «после первого кадра»**. `preUpdate` в p9/p10
ни разу не вызывался: ни верификация, ни init, ни per-frame путь — ничего.
Поэтому p10 не падал (мир чёрный = 0 тиков, звук заставки = sound channel
независим), а фраза «меню чуть едет» про наше меню невозможна (оно hidden и
мертвое) — имелся в виду HUD/заставка игры.

**p11 — первая сборка, где `preUpdate` реально побежал** (60 Hz, без timestep):
первый кадр мира → catch-up (cap 5 тиков) → **ленивая верификация + исполнение
init меню**. И init упёрся в:

```
new Bitmap()          # bitmapData == null, baseWidth == 0
width  = 36           # setter: scaleX = 36 / 0 = Infinity
height = 36           # scaleY = 36 / 0 = Infinity
```

В AIR это роняет **процесс** (нетворчбл/нативное деление на ноль в setter) →
«нажал старт — закрылось полностью». Это ровно мертвый конец №20, который
предыдущий агент записал, но код p9/p10 его нарушал (`add_bmp` стоял width/height
сразу после `new Bitmap()`), а до p11 этот код ни у кого не исполнялся — поэтому
никто этого не видел.

Почему p7 жив: в p7 **вообще нет Bitmap** (UI только TextField; пустой
TextField = 100×100 по умолчанию, деления на ноль нет).

**patch12**: width/height в `add_bmp` только после присвоения bitmapData
(иконки боссов/мобов — если ключ найден; слоты сетки предметов — 0×0 до
`do_fill`, где порядок и так верный: bitmapData → width/height → visible).
Остальное = p11 (без timestep, 60 FPS).

**Полное сравнение оригинала (main) с патчами** (все теги, все тела методов,
RECT, версия): p11/p12 отличаются от оригинала **только** телом 12
(`Preloader.showError`, same-length) и телом 825 (`SurvivalGame.preUpdate`);
p10 дополнительно телом 269 (`Core.onEnterFrame`, timestep). RECT `78 00 07 d0
00 00 19 00 00` (800×640) и версия 31 везде одинаковые. Сборка p10 из текущего
tools = байт-в-байт закатанный p10 (blob 93eac263…) — конвейер верен.

---

# 11. ПЛАН ДЛЯ НОВОГО ЧАТА (ПОРЯДОК, НЕ ПРЫГАТЬ)

Статус на 2026-09-12 (чат 3, к вечеру): шаги 1–5 выполнены; **p11 у пользователя
пал при «старт»** (процесс закрылся) — причина установлена (10.2: width/height на
пустом Bitmap; в p9/p10 код меню не исполнялся вообще). **patch12 собран,
проверен (0 ошибок верификатора, хвост байт-в-байт, контейнер нетронут), ждём
тест: старт проходит, мир виден, меню открывается Insert/F7.** Дальше: 120 FPS
аккумулятором (шаг 6), F7 в лобби, каталоги лениво, свет/ход.
