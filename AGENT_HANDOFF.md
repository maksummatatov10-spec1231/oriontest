...
| `arena/01a0913d-oriontest` | **единственная** ветка этой Arena-сессии. Весь труд только здесь. |
...
3. Сравнить `orion_menu_patch7.swf` vs 9 vs 10: байты 33–53 и 66–81 Core.onEnterFrame; code_len preUpdate; visible на старте.  
4. Дизасм OrionScreen cinit (размеры). WorldPreview.update. setCutsceneMode.  
5. **Эксперимент 1:** выключить `patch_core_timestep` (закомментировать вызов в `patch_one`), FPS header 60, меню как сейчас или проще. Собрать, закоммитить, **дать raw-ссылку**, спросить про мир.  
6. Если мир появился — timestep виноват. 120 FPS делать иначе: не nop first tick; не prev=local2 вслепую. Изучить цикл, интернет (accumulator), но сохранить безусловный tick или скорректировать elapsed так, чтобы while давал 1 тик на 16.6мс.  
