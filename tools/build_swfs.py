...
def build_code(abc: Abc, orig_code: bytes, experimental: bool) -> bytes:
    ...
    items_mn = n("ITEMS")
    durability_mn = n("durability")
    jumpPower_mn = n("jumpPower")
    maxJumpHeight_mn = n("maxJumpHeight")
    gravity_mn = n("gravity")
    ...
    s_t5 = abc.intern_string("Игрок")
    s_pg5 = abc.intern_string("pg5")
    s_str = abc.intern_string("Стр.")
    s_tfPage = abc.intern_string("tfPage")
    s_holdSlot = abc.intern_string("holdSlot")
    s_holdCd = abc.intern_string("holdCd")
    s_holdGap = abc.intern_string("holdGap")
    s_icidpre = abc.intern_string("icid")
    s_ph = abc.intern_string("Редактор игрока")
    s_jp = abc.intern_string("Сила прыжка")
    s_mjh = abc.intern_string("Выс. прыжка")
    s_grav = abc.intern_string("Гравитация")
    s_imm = abc.intern_string("Бессмертие: ВЫКЛ (нажми)")
    s_imm_on = abc.intern_string("Бессмертие: ВКЛ (нажми)")
    s_plHp = abc.intern_string("plHp")
    s_plSpd = abc.intern_string("plSpd")
    s_plJp = abc.intern_string("plJp")
    s_plMj = abc.intern_string("plMj")
    s_plGr = abc.intern_string("plGr")
    s_plLv = abc.intern_string("plLv")
    s_plImm = abc.intern_string("plImm")
    ...
    def set_page_vis(active):
        for _idx, _sidx in enumerate((s_pg1, s_pg2, s_pg3, s_pg4, s_pg5), start=1):
            dget(_sidx)
            a.coerce_a()
            a.setlocal(L_TMP)
            a.getlocal(L_TMP)
    ...
    add_tf(s_str, 246, 60, 44, 20, loc=L_PG)
    add_tf(s_1, 292, 58, 26, 22, store=s_tfPage, loc=L_PG)
    ...
    loadpage(s_pg5)
    fill(COL_BG, 0, HEAD_Y, W, H - HEAD_Y, 0, loc=L_PG)
    add_tf(s_ph, 16, 62, 408, 20, loc=L_PG)
    _PL = [(s_hp, s_100, s_plHp, s_100), (s_spd, s_2, s_plSpd, s_2),
           (s_jp, s_2, s_plJp, s_2), (s_mjh, s_2, s_plMj, s_2),
           (s_grav, s_2, s_plGr, s_2), (s_lvl, s_1, s_plLv, s_1)]
    for _n, (_lab, _def, _st, _dt) in enumerate(_PL):
        _y = 92 + _n * 30
        add_tf(_lab, 16, _y, 104, 22, loc=L_PG)
        add_tf(_def, 122, _y, 116, 22, store=_st, inp=True, def_text=_dt, loc=L_PG)
        fill(COL_BTN, 248, _y - 2, 80, 24, 5, loc=L_PG)
        add_tf(s_ok, 268, _y, 44, 20, loc=L_PG)
    fill(COL_RED, 16, 278, 408, 26, 6, loc=L_PG)
    add_tf(s_imm, 28, 281, 384, 22, store=s_plImm, loc=L_PG)

    # ---- tab labels ----
    for _i, _s in enumerate((s_t0, s_t1, s_t2, s_t3, s_t4, s_t5)):
    ...
    a.getlocal(L_FMT)
    a.getlocal(L_FMT)
    a.getproperty(durability_mn)
    a.getlocal(L_TMP)
    a.constructprop(stack_cls, 3)
    ...
    for _i in range(IC_PER):
        ...
        a.setproperty_l(star_mn)
        a.getlocal(L_MENU)
        a.pushstring(s_holdCd)
        a.pushbyte(0)
        a.setproperty_l(star_mn)
        a.getlocal(L_MENU)
        a.pushstring(s_holdGap)
        a.pushbyte(12)
        a.setproperty_l(star_mn)
        a.jump_to("click_done")
        a.label(_miss)
    ...
    a.convert_i()
    a.pushbyte(5)
    a.ifne("no_player_page")
    _PL2 = [(s_plHp, health_mn), (s_plSpd, speed_mn), (s_plJp, jumpPower_mn),
            (s_plMj, maxJumpHeight_mn), (s_plGr, gravity_mn), (s_plLv, level_mn)]
    for _n, (_st, _prop) in enumerate(_PL2):
        _y = 92 + _n * 30
        _miss = f"plm{_n}"
        hit(a, L_MX, L_MY, 248, _y - 2, 80, 24, _miss)
        need_player("click_done")
        read_tf(_st)
        a.getlocal(L_PL)
        a.getlocal(L_TMP)
        ...
        a.jump_to("click_done")
        a.label(_miss)
    hit(a, L_MX, L_MY, 16, 278, 408, 26, "click_done")
    dget(s_plImm)
    a.convert_b()
    a.iftrue("imm_to_false")
    a.pushtrue()
    a.jump_to("imm_toggled")
    a.label("imm_to_false")
    a.pushfalse()
    a.label("imm_toggled")
    a.setlocal(L_TMP)
    a.getlocal(L_MENU)
    a.pushstring(s_plImm)
    a.getlocal(L_TMP)
    a.setproperty_l(star_mn)
    a.getlocal(L_PL)
    a.pushnull()
    ...
    a.getlocal(L_TMP)
    a.convert_b()
    a.iffalse("imm_lbl_off")
    a.pushstring(s_imm_on)
    a.jump_to("imm_lbl_done")
    a.label("imm_lbl_off")
    a.pushstring(s_imm)
    a.label("imm_lbl_done")
    a.setproperty(text_mn)
    a.label("no_player_page")
    ...
    a.add()
    a.setlocal(L_TMP)
    dset_local(s_holdCd, L_TMP)
    a.getlocal(L_TMP)
    dget(s_holdGap)
    a.convert_i()
    a.iflt("nohold")
    a.getlocal(L_MENU)
    a.pushstring(s_icidpre)
    dget(s_holdSlot)
    a.convert_i()
    a.add()
    a.getproperty_l(star_mn)
